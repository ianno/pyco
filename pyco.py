...
parser.add_argument('-m', '--minimize',
                    choices=['size', 'ports', 'cost'], default='ports',
...
if __name__ == "__main__":
    ...
    if args.facs is not None:
        if args.facs != 'all':
            ...
            pytest.main(['-s', 'pyco-dev/tests/test_eps_facs.py::test_synth_6_10_dc_%sspec' % args.facs])
        elif args.facs == 'all':
    ...
    if args.edg is not None:
        if args.edg == '1':
            print 'Running EDG test in pyco-dev/tests/test_edg_motor.py::test_ltl_spec_med...\n'
            pytest.main(['-s', 'pyco-dev/tests/test_edg_motor.py::test_ltl_spec_med'])
